       ...
       do j = 1, cDFT_NAtoms(i)
          ...
          if (inode == ionode .AND. iprint_SC > 1) then
               ...
               if (cDFT_k > ni_in_cell) then
                   ...
                   call cq_abort('Error: Atom index ' // trim(adjustl(k_str)) // &
                                 ' has exceeded total atoms (' // & 
                                 trim(adjustl(ni_in_cell_str)) // ').')
               end if
               if (cDFT_AtomMask(cDFT_k)) then
                   write (io_lun, fmt='(2x, "Error: Duplicate atom index found:", i6)') &
                          cDFT_k
                   call cq_abort('Error: Dumplicate atom index detected: ')
               else
